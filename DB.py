...
async def save_user_pref(user_id: int, prod_time: str, notes: str):
    async with get_db_connection() as conn:
        query = """
...
async def add_tasks(user_id: int, tasks: list[str]):
    '''
    В таблице бд есть колонка приоритета, нужно заставить пользователя
    вводить приоритеты. Также нужно поработать с форматом отправленных задач. 
    ----- 
    может добавить парсер по обработки текст задач 
    '''
    async with get_db_connection() as conn:
        query = """INSERT INTO tasks (user_id, title) VALUES ($1, $2)"""
        await conn.execute(query, user_id, title)
